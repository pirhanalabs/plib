...
class PixelOutline extends h2d.filter.Shader<InternalShader>
{
	...
	public function new(color:Int = 0x0, a = 1.0, knockOut = false)
	{
	...
	{
		alphaMul = hxd.Math.clamp(alphaMul, 0, 1);
		knockOut = alphaMul < 1;
	...
	static var SRC =
		{
			...
			function fragment()
			{
				...
				var onEdge = ( // Get outline color multiplier based on transparent surrounding pixels
					(1 - curColor.a) * max(texture.get(vec2(input.uv.x + texelSize.x, input.uv.y)).a * leftMul, // left outline
